...
'''
    4. TF-IDF:
        - article title 
'''
print('Start preparing TFIDF-based model...')
...
#------------------------------------------------------------------------------------------
# Comparison results
comparison_results = pd.DataFrame([TFIDF_df_metrics.mean(axis = 0, skipna = True)], 
                                index=['TFIDF_mean']) 

print('Comparison results for top-{} recommendaitons ...'.format(k))
print(comparison_results)

output_filename = '2_comparison_results_TFIDF.csv'
comparison_results.to_csv(output_filename)

print('Complete running TFIDF-based recommendation models in {} seconds...'.format(round(time.time() - top_time,2)))
